  ...
  <div class="uni-page">
      ...
      <div class="page-hero-inner">
        ...
        <div class="hero-facts">
          <div class="hero-fact">
            ...
            <span class="hero-fact-label">{{ universityWord(totalUniversities) }}</span>
          </div>
    ...
    <div class="page-layout">
      ...
      <aside class="filters-sidebar" :class="{ open: filtersOpen }">
        ...
        <div class="filter-block">
          <label class="filter-label">Тип вуза</label>
          <div class="chip-list">
            <button
        ...
        <div class="filter-block">
          <div class="filter-label-row">
            <label class="filter-label">Проходной балл (бюджет)</label>
      ...
      <main class="main-panel">
        ...
        <section class="map-section" :class="{ 'is-city-mode': !!selectedMapCity }">
          ...
          <div class="map-selected-chip" v-if="selectedMapCity">
            ...
            <span class="map-selected-count">{{ cityUniCount(selectedMapCity) }} {{ universityWord(cityUniCount(selectedMapCity)) }}</span>
        ...
        <section class="list-section" id="uni-list-section">
          <div class="list-toolbar desktop-only">
            <h2 class="list-title">
              <template v-if="selectedMapCity">Вузы · {{ selectedMapCity }}</template>
              <template v-else>Список вузов</template>
          ...
          <div v-if="!loading && !error && !selectedMapCity" class="empty-state">
            <i class="bi bi-map"></i>
            <p>Выберите город на карте — ниже появится список университетов этого города.</p>
          </div>

          <div v-else-if="!loading && !error && selectedMapCity && filteredList.length === 0" class="empty-state">
            <i class="bi bi-search"></i>
            <p>В городе «{{ selectedMapCity }}» вузов по текущим фильтрам не найдено.</p>
              ...
              <div class="card-body">
                ...
                <div class="card-actions">
                  <button
                    ...
                    type="button"
                    class="btn-primary"
                    @click.stop="$router.push(`/UniversityAbout/${uni.id}`)"
                  >
                    Подробнее
                  </button>
                  <button
    ...
    <transition name="drawer">
      <div v-if="showCompare" class="compare-drawer">
        ...
        <div v-if="!compareList.length" class="empty-state compact">
          <p>Добавьте вузы кнопкой «В сравнение»</p>
        </div>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td v-for="u in compareList" :key="'a'+u.id">
                  <button type="button" class="btn-primary sm" @click="$router.push(`/UniversityAbout/${u.id}`)">
                    Подробнее
                  </button>
                </td>
              </tr>
...
const PER_PAGE = 9;
const COMPARE_KEY = 'torap_compare_unis';

const OWNERSHIP = [
  { value: 'state', label: 'Государственные' },
  { value: 'private', label: 'Частные' },
  { value: 'research', label: 'Национальные исследовательские' },
];
...
/** Демо-вузы для городов без данных в API (примерная логика) */
const DEMO_BY_CITY = {
  Алматы: [
    { name: 'Казахский национальный университет имени аль-Фараби', directions: 'Гуманитарное', ownership: 'research' },
    { name: 'КазНТУ имени К.И. Сатпаева', directions: 'Техническое', ownership: 'research' },
    { name: 'Университет КИМЭП', directions: 'Экономика', ownership: 'private' },
    { name: 'Алматинский технологический университет', directions: 'IT', ownership: 'state' },
  ],
  Шымкент: [
    { name: 'Южно-Казахстанский университет имени М. Ауэзова', directions: 'Техническое', ownership: 'state' },
    { name: 'Международный казахско-турецкий университет', directions: 'Гуманитарное', ownership: 'private' },
  ],
  Караганда: [
    { name: 'Карагандинский технический университет', directions: 'Техническое', ownership: 'state' },
    { name: 'Медицинский университет Караганды', directions: 'Медицина', ownership: 'state' },
  ],
  Актобе: [
    { name: 'Западно-Казахстанский медицинский университет', directions: 'Медицина', ownership: 'state' },
    { name: 'Актюбинский региональный университет', directions: 'Гуманитарное', ownership: 'state' },
  ],
  Атырау: [
    { name: 'Атырауский университет нефти и газа', directions: 'Техническое', ownership: 'state' },
  ],
  Павлодар: [
    { name: 'Торайгыров университет', directions: 'IT', ownership: 'state' },
  ],
  'Усть-Каменогорск': [
    { name: 'Восточно-Казахстанский технический университет', directions: 'Техническое', ownership: 'state' },
  ],
  Семей: [
    { name: 'Университет имени Шакарима', directions: 'Гуманитарное', ownership: 'state' },
  ],
  Костанай: [
    { name: 'Костанайский региональный университет', directions: 'Гуманитарное', ownership: 'state' },
  ],
  Петропавловск: [
    { name: 'Северо-Казахстанский университет', directions: 'IT', ownership: 'state' },
  ],
  Тараз: [
    { name: 'Таразский региональный университет', directions: 'Техническое', ownership: 'state' },
  ],
  Уральск: [
    { name: 'Западно-Казахстанский университет имени Утемисова', directions: 'Гуманитарное', ownership: 'state' },
  ],
  Кызылорда: [
    { name: 'Кызылординский университет имени Коркыт Ата', directions: 'IT', ownership: 'state' },
  ],
  Актау: [
    { name: 'Каспийский университет технологий и инжиниринга', directions: 'Техническое', ownership: 'state' },
  ],
};
...
export default {
  ...
  data() {
    return {
      ...
      costBounds: { min: 300000, max: 2500000 },
      scoreBounds: { min: 50, max: 140 },
  ...
  computed: {
    ...
    /** Вузы для маркеров: только в режиме города (на силуэте КЗ — лишь пины городов) */
    mapUniversities() {
      if (!this.selectedMapCity) return [];
  ...
  watch: {
    ...
    page() {
      const el = document.getElementById('uni-list-section');
  ...
  methods: {
    ...
    selectMapCity(cityName, { fly = true, scrollList = true } = {}) {
      ...
      if (fly && this.map) {
        this.$nextTick(() => {
          if (!this.mapAlive) return;
          // переключаемся на уличную карту города + пины вузов
    ...
    /**
     * Режим города: OSM-улицы, зум на весь город, все вузы на карте.
     */
    ...
    exitCityStreetMode() {
      ...
      try {
        ...
        // 4) убрать пины вузов города + нарисовать пины городов
    ...
    /** Bounds города по вузам + запас, либо вокруг центра */
    ...
    classifyOwnership(name = '') {
      ...
      if (
        ...
        n.includes('казахстанско-') ||
        n.includes('кимэп') ||
        n.includes('kimep') ||
        n.includes('туркестан') ||
        n.includes('частн')
      ) {
        return 'private';
      }
    ...
    extractTags(raw) {
      ...
      (raw.specializations || []).slice(0, 4).forEach((s) => {
    ...
    universityWord(n) {
      const m10 = n % 10;
      const m100 = n % 100;
      if (m100 >= 11 && m100 <= 14) return 'университетов';
      if (m10 === 1) return 'университет';
      if (m10 >= 2 && m10 <= 4) return 'университета';
    ...
    async fetchInstitutions() {
      ...
      try {
        ...
        const real = list
          .filter((i) => i.type && i.type.toLowerCase() === 'university')
          .map((i) => this.enrichUniversity(i));
        ...
        // Демо-вузы по другим городам (примерная логика, пока в API мало городов)
        const demo = this.buildDemoUniversities(real);
        ...
        console.error(e);
        this.error = 'Не удалось загрузить университеты. Проверьте, что API запущен.';
      } finally {
        this.loading = false;
      }
    },
    ...
    buildDemoUniversities(existing) {
      ...
      Object.entries(DEMO_BY_CITY).forEach(([cityName, unis]) => {
        ...
        unis.forEach((item, idx) => {
          ...
          const enriched = this.enrichUniversity(raw);
          if (item.ownership) {
    ...
    renderCityPins() {
      ...
      // Когда город выбран — только маркеры вузов
      if (this.selectedMapCity) return;
    ...
    syncMarkers() {
      ...
      const list = this.mapUniversities;
    ...
    popupHtml(u) {
      ...
      const link = u.isDemo
        ...
        : `<a href="/UniversityAbout/${u.id}" class="popup-link">Подробнее →</a>`;
      return `
        <div class="popup-inner">
          <img src="${img}" alt="" class="popup-logo" onerror="this.style.display='none'"/>
          <div>
            <strong>${u.name}</strong> ${demoBadge}
            <div class="popup-meta">${this.cityOf(u)} · балл ${u.passScore}</div>
            ${link}
          </div>
        </div>
      `;
    ...
    scrollToCard(id) {
      ...
      this.$nextTick(() => {
        const el = document.getElementById(`uni-card-${id}`);
