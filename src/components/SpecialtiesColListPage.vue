  ...
  <div class="spec-page">
    <header class="page-hero">
      ...
      <div class="page-hero-inner">
        <h1>Специальности <span class="hero-accent">колледжей</span></h1>
        <p class="page-sub">
          Выберите направление — затем квалификацию и конкретную специальность
        </p>
        <div class="hero-facts">
          <div class="hero-fact">
            <span class="hero-fact-value">{{ collegeSpecialties.length || '—' }}</span>
            <span class="hero-fact-label">{{ specialtiesWord(collegeSpecialties.length) }}</span>
          </div>
    ...
    <div class="page-layout">
      ...
      <main class="main-panel">
        ...
        <div v-else-if="error" class="state-box error">
          ...
          <span>{{ error }}</span>
          <button type="button" class="btn-retry" @click="fetchCollegeSpecialties">Повторить</button>
        </div>
          ...
          >
            <div class="spec-card-body">
              ...
              <div class="spec-card-foot">
                <span v-if="qualsOf(specialty).length" class="spec-count">
                  {{ qualsOf(specialty).length }}
                  {{ qualWord(qualsOf(specialty).length) }}
                </span>
                <span class="spec-cta">
                  Смотреть
...
export default {
  ...
  computed: {
    totalQualifications() {
      return this.collegeSpecialties.reduce(
        (sum, s) => sum + this.qualsOf(s).length,
        0
      );
    },
    filteredSpecialties() {
      const q = this.searchQuery.trim().toLowerCase();
      if (!q) return this.collegeSpecialties;
      return this.collegeSpecialties.filter(
        (s) =>
  ...
  methods: {
    /** Квалификации направления — API может отдавать ключ в разных форматах */
    qualsOf(specialty) {
      return (
        specialty.collegeQualifications ||
        specialty.college_qualifications ||
    ...
    async fetchCollegeSpecialties() {
      ...
      try {
        const response = await axios.get('http://localhost:8000/api/specialties', {
          params: { type: 'college', include: 'collegeQualifications' },
        });
    ...
    navigateToQualifications(specialtyId) {
      // Сохраняем данные о выбранной специальности — их читает страница квалификаций
