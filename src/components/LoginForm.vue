  ...
  setup() {
    ...
    const handleLogin = async () => {
      try {
        ...
        // Перенаправление на главную страницу с обновлением
        setTimeout(() => {
          router.push('/').then(() => {
            window.location.reload(); // Обновляем страницу после перенаправления
          });
        }, 1500);
      } catch (error) {
        password.value = '';
    ...
    const handleRegister = async () => {
      try {
        ...
        localStorage.setItem('token', response.data.access_token);
        registerSuccessMessage.value = '✅ Регистрация успешна!';
        ...
        registerErrorMessage.value = `❌ ${error.response?.data?.error || error.message}`;
      }
    };
